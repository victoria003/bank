...
// ----------------------------------------------------------------------------
// Simple CRUD API for `APP_RECORDS` table
// ----------------------------------------------------------------------------
...
app.get('/api/records', authenticateToken, async (req, res) => {
  if (!hasSnowflakeCredentials()) {
    return res.json(recordsInMemory);
  }
...
app.get('/api/records/:id', authenticateToken, async (req, res) => {
  ...
  if (!hasSnowflakeCredentials()) {
    const found = recordsInMemory.find(r => r.id === id);
  ...
  try {
    await ensureRecordsTableExists();
    const rows = await runQuery(`SELECT id, name, email, details, TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS') as created_at FROM ${RECORDS_TABLE} WHERE id = ?`, [id]);
    const r = rows[0];
    if (!r) return res.status(404).json({ error: 'Not found' });
    res.json({ id: r.ID || r.id, name: r.NAME || r.name, email: r.EMAIL || r.email, details: r.DETAILS || r.details, created_at: r.CREATED_AT || r.created_at });
  } catch (err) {
...
app.post('/api/records', authenticateToken, async (req, res) => {
  ...
  if (!hasSnowflakeCredentials()) {
    const newRec = { id, name, email, details: details || '', created_at: new Date().toISOString() };
    recordsInMemory.unshift(newRec);
    return res.status(201).json(newRec);
  }

  try {
    await ensureRecordsTableExists();
    await runQuery(`INSERT INTO ${RECORDS_TABLE} (id, name, email, details) VALUES (?, ?, ?, ?)`, [id, name, email, details || '']);
    const rows = await runQuery(`SELECT id, name, email, details, TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS') as created_at FROM ${RECORDS_TABLE} WHERE id = ?`, [id]);
...
app.put('/api/records/:id', authenticateToken, async (req, res) => {
  ...
  if (!hasSnowflakeCredentials()) {
    const idx = recordsInMemory.findIndex(r => r.id === id);
    if (idx === -1) return res.status(404).json({ error: 'Not found' });
    recordsInMemory[idx] = { ...recordsInMemory[idx], name, email, details: details || recordsInMemory[idx].details };
    return res.json(recordsInMemory[idx]);
  }

  try {
    await ensureRecordsTableExists();
    await runQuery(`UPDATE ${RECORDS_TABLE} SET name = ?, email = ?, details = ? WHERE id = ?`, [name, email, details || '', id]);
    const rows = await runQuery(`SELECT id, name, email, details, TO_CHAR(created_at, 'YYYY-MM-DD HH24:MI:SS') as created_at FROM ${RECORDS_TABLE} WHERE id = ?`, [id]);
    res.json(rows[0]);
  } catch (err) {
...
app.delete('/api/records/:id', authenticateToken, async (req, res) => {
  ...
  if (!hasSnowflakeCredentials()) {
    recordsInMemory = recordsInMemory.filter(r => r.id !== id);
    return res.json({ success: true });
  }

  try {
    await ensureRecordsTableExists();
    await runQuery(`DELETE FROM ${RECORDS_TABLE} WHERE id = ?`, [id]);
